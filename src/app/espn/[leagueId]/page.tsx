...
export default async function EspnLeaguePage({ params, searchParams }: PageProps) {
  ...
  try {
    ...
    const userRoster = base.rosters.find((roster) =>
      selectedTeamId != null ? roster.roster_id === selectedTeamId : Boolean(credentialOwner && identity(roster.owner_id) === credentialOwner)
    );
    ...
    const topWaivers = waiverBoard.filter((candidate) => (candidate.netRosterGain ?? candidate.valueOverReplacement ?? 0) > 0).slice(0, 5);
    const publicMode = !credentials.swid || !credentials.espnS2;
    ...
    return <div className="page-wrap"><div className="error-banner">{message} WAR ROOM will not fabricate ESPN data.</div><Link href="/connect" className="connect-button">Reconnect ESPN league</Link></div>;
  }
}
