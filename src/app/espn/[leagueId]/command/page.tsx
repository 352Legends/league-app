...
async function loadEspnMissionControl(args: {
  leagueId: string;
  season: number;
  teamId: number;
  credentials: EspnCredentials;
}) {
  const { leagueId, season, teamId, credentials } = args;
  try {
    ...
    if (!userRoster) {
      return { ok: false as const, message: `WAR ROOM could not match ESPN team ${teamId} to this league.` };
    }
    ...
    const matchupWeeks = Array.from(
      { length: Math.max(1, playoffWeekStart - base.state.week) },
      (_, index) => base.state.week + index,
    );
    ...
    const opponentTeam = opponentRosterId == null
      ? null
      : baselineImpact.baseline.teams.find((team) => team.rosterId === opponentRosterId) ?? null;
    const thisWeekWin = opponentTeam
      ? winProbability(userTeam.projectedMean, userTeam.projectedSd, opponentTeam.projectedMean, opponentTeam.projectedSd)
      : null;
    const actionableBreakouts = breakoutRadar.filter(
      (candidate) => candidate.ownership === "AVAILABLE" && ["ADD NOW", "STASH"].includes(candidate.action),
    );
  ...
  if (!Number.isInteger(teamId)) {
    return (
      <div className="page-wrap">
        <div className="error-banner">Choose your ESPN team before opening Mission Control.</div>
        <Link href={`/espn/${leagueId}?season=${season}`} className="connect-button">Choose team</Link>
      </div>
    );
  }

  const result = await loadEspnMissionControl({ leagueId, season, teamId, credentials });
  if (!result.ok) {
    return (
      <div className="page-wrap">
        <div className="error-banner">{result.message} WAR ROOM will not fabricate ESPN recommendations.</div>
        <Link href={`/espn/${leagueId}?season=${season}&teamId=${teamId}`} className="connect-button">Back to ESPN league</Link>
      </div>
    );
  }
    ...
    <div className="page-wrap">
      ...
      <section className="metric-grid command-summary">
        <article className="metric-card metric-card--positive">
          ...
          <strong className="metric-value">{userTeam.championshipProbability.toFixed(1)}%</strong>
          <p className="metric-detail">ESPN league-specific title probability</p>
        </article>
        <article className="metric-card">
          <p className="eyebrow">THIS WEEK WIN</p>
          <strong className="metric-value">{thisWeekWin == null ? "—" : `${thisWeekWin.toFixed(0)}%`}</strong>
          <p className="metric-detail">{opponentTeam ? `vs ${opponentTeam.teamName}` : "Current matchup unavailable"}</p>
        </article>
        <article className="metric-card">
          <p className="eyebrow">ALPHA OPPORTUNITIES</p>
          <strong className="metric-value">{actionableBreakouts.length}</strong>
          <p className="metric-detail">NFL evidence; ESPN market factor neutral</p>
        </article>
        <article className="metric-card">
          <p className="eyebrow">URGENT DECISIONS</p>
          <strong className="metric-value">{urgent}</strong>
          <p className="metric-detail">Priority actions with urgency ≥75</p>
        </article>
      </section>

      <section className="section-block command-priority-section">
        <div className="section-heading">
          <div><p className="eyebrow">WAR ROOM PRIORITIES · ESPN</p><h2>Ranked by Δ Championship Probability</h2></div>
          <span className="status-chip">LINEUP + WAIVERS + ALPHA + TRADES</span>
        </div>
        <div className="command-model-note">
          <strong>PROVIDER-NORMALIZED</strong>
          <span>ESPN supplies league rules, rosters, standings and schedule. Canonical NFL evidence supplies player performance and matchup modeling. Missing ESPN market/activity signals are left neutral rather than copied from another provider.</span>
        </div>
        {top.length ? (
                ...
                <div className="command-priority-main">
                  <div className="command-priority-title">
                    ...
                    <div className="command-delta">
                      ...
                      <small>{candidate.horizon === "ONE_WEEK" ? "current-week only" : "sustained impact"}</small>
                    </div>
                  </div>
                  <div className="command-move-metrics">
                    ...
                    <div><span>URGENCY</span><strong>{candidate.urgency.toFixed(0)}</strong><small>timing signal</small></div>
                    <div><span>PRIORITY SCORE</span><strong>{candidate.priorityScore.toFixed(1)}</strong><small>cross-decision rank</small></div>
                  </div>
          ...
          <div className="lineup-clear"><strong>NO QUANTIFIED UPGRADE FOUND</strong><span>No ESPN lineup, waiver or trade action cleared the positive-value evidence threshold.</span></div>
        )}
      </section>

      <section className="section-block">
        <div className="section-heading">
          <div><p className="eyebrow">DECISION INVENTORY</p><h2>What ESPN Mission Control evaluated</h2></div>
          <span className="status-chip">NORMALIZED PROVIDER DATA</span>
        </div>
